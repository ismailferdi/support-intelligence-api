...
def test_analyze_returns_200_with_well_formed_response(
    ...
    response = client.request(
        "GET",
        "/tickets/analyze",
        json=sample_ticket.model_dump(exclude_none=True),
    )
...
def test_analyze_returns_503_when_ticket_save_fails(
    monkeypatch, sample_ticket
):
...
def test_analyze_returns_503_when_analysis_save_fails(
    monkeypatch, sample_ticket
):
    engine = _bare_engine()
...
def test_analyze_returns_503_when_failure_record_save_fails(
    monkeypatch, sample_ticket
):
    engine = _bare_engine()
    Ticket.__table__.create(engine)
